    ...
    <MobileCardBase class="overflow-hidden">
      <div class="bg-[#145C34] px-4 py-3.5 flex justify-between items-center">
        <div class="flex items-center gap-3 text-white">
          ...
          <div>
            <p class="text-sm font-bold">{{ displayDateRange }}</p>
            <p class="text-[11px] opacity-80">{{ displayDuration }}</p>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div class="fixed inset-0 z-[100] flex justify-center items-end" v-if="isModalOpen">
        ...
        <transition name="slide-up" appear>
          <div class="relative w-full max-w-md bg-white rounded-t-3xl pt-4 pb-6 px-0 shadow-2xl z-10 flex flex-col h-[85vh]"
            @click.stop>
            ...
            <div class="flex justify-between items-center px-6 mb-4 shrink-0">
              <div class="w-12"></div>
              <h3 class="font-bold text-[17px] text-gray-900">Set Date</h3>
              <button @click="closeCalendarModal"
                class="w-12 text-right text-sm font-bold text-blue-600 hover:text-blue-800 transition">
                Batal
              </button>
            </div>

            <div class="grid grid-cols-7 text-center border-y border-gray-100 py-2 mb-4 shrink-0">
              <div v-for="(day, index) in daysOfWeek" :key="day" class="text-xs font-bold"
                :class="index === 0 ? 'text-red-500' : 'text-gray-800'">
                {{ day }}
              </div>
            </div>
            ...
            <div class="px-6 overflow-y-auto flex-1 scrollbar-hide">
              ...
              <div class="flex justify-between items-center mb-6 mt-2">
                ...
                <div class="flex gap-2">
                  <button @click="prevMonth"
                    class="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 transition">
              ...
              <div class="grid grid-cols-7 gap-y-3">
                <div v-for="day in calendarDays" :key="day.date.toISOString()"
                  class="relative flex justify-center items-center h-10">

                  <div v-if="isInRange(day.date)" class="absolute inset-0 bg-[#E8F5E9] w-full h-full" :class="{
                    'rounded-l-full': isStartDate(day.date),
                    'rounded-r-full': isEndDate(day.date)
                  }">
                  </div>
...
const calendarDays = computed(() => {
	...
	// Padding Next Month (to complete 6 rows = 42 cells)
...
const formattedStartDate = computed(() => {
	return startDate.value ? formatDateString(startDate.value) : '-'
})

const formattedEndDate = computed(() => {
	return endDate.value ? formatDateString(endDate.value) : '-'
})
...
const selectDate = (date) => {
	// If nothing is selected, or both are already selected (reset flow)
	if (!startDate.value || (startDate.value && endDate.value)) {
		startDate.value = date
		endDate.value = null
	}
	// If start is selected but clicked date is BEFORE start date -> Reassign start date
	else if (date < startDate.value) {
		startDate.value = date
	}
	// If start is selected and clicked date is AFTER start date -> Assign end date
	else {
		endDate.value = date
	}
}

// Logic Checks for UI Styling
...
const formatDateString = (date) => {
	...
	return `${d} ${m} ${y}` // Output: 07 Jun 2026
}

const formatShortDate = (date) => {
	const d = date.getDate()
	const m = monthNames[date.getMonth()].substring(0, 3)
	return `${d} ${m}` // Output: 25 May
}
</script>
